...
export default function Appendix() {
  ...
  useEffect(() => {
    const h = (e: KeyboardEvent) => { if (e.key === 'Escape' && selectedDoc) setSelectedDoc(null); };
  ...
  const openDoc = (code: string) => {
    const latin = code
      .replace('КР', 'KR').replace('УВ', 'UV').replace('ОТВ', 'OTV').replace('СП', 'SP')
      .replace('РП', 'RP').replace('СЗ', 'SZ').replace('ЗП', 'ZP').replace('ТР', 'TR')
      .replace('ПРЧ', 'PRCH').replace('ПРК', 'PRK').replace('СПР', 'SPR')
      .replace('ОБ', 'OB').replace('АКТ', 'AKT').replace('РД', 'RD').replace('ОП', 'OP')
      .replace('ЗКЛ', 'ZKL').replace('ПРД', 'PRD').replace('РШ', 'RSH').replace('ЛО', 'LO')
  ...
  return (
    <PageTransition className="wiki-content">
      ...
      <DocForm doc={selectedDoc} onClose={() => setSelectedDoc(null)} />
    </PageTransition>
  );
}